...
namespace cadmium::Neuron_Space {
	...
	struct NeuronState {
		...
		NeuronState(): sigma(0), ___current_state___(Neuron_States::Forward_Pass), activation_function("sigmoid"), weights({}), input_values({}), d_weights({}), weighted_sum(0), inputs_received(0), prediction(0){};
	};
	...
	class Neuron: public Atomic<NeuronState> {
		...
		public:
			...
			vector<Port<double>> backward_outputs;
			 ...
			 */
			Neuron(const string& id, int num_inputs, int num_outputs, double lr_in, string activation_function_in): Atomic<NeuronState>(id, NeuronState()) {
				...
				if (num_inputs < 1){
					...
					for (int i = 0; i < num_inputs; i++){
						...
						backward_outputs.push_back(addOutPort<double>("BOutput"+to_string(i)));
						state.weights.push_back(probDist(generator));
						state.d_weights.push_back(0);
						state.input_values.push_back(0);
					}
					state.weights.push_back(probDist(generator));
					state.d_weights.push_back(0);
					state.input_values.push_back(bias);
				}
			...
			void externalTransition(NeuronState& state, double e) const override {
				...
				if (state.___current_state___ == Neuron_States::Forward_Pass){
					...
					if (state.inputs_received == backward_inputs.size()){
						...
						if (state.activation_function.compare("sigmoid") == 0){
							double d_sigmoid = state.prediction * (1-state.prediction);
							state.error = state.error*d_sigmoid;
							for (int i = 0; i < state.weights.size(); i++){
								state.d_weights[i] = state.error*state.input_values[i];
							}
			...
			void output(const NeuronState& state) const override {
				switch(state.___current_state___){
					...
					case Neuron_States::Backward_Pass:
						break;
					case Neuron_States::Updating:
						for (int i = 0; i < backward_outputs.size(); i++){
							backward_outputs[i]->addMessage(state.error*state.weights[i]);
						}
						break;
					default:
						assert(("Not a valid state", false));
